...
@section('title', 'Editar Permiso - SGPD COAC')
    ...
    <div class="bg-white rounded-lg border border-gray-200 mb-6 p-5">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-gray-600 mt-1">Modificar los datos del permiso: {{ $permission->code }}</p>
            </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        rows="5"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                        placeholder="Describe el propósito y alcance de este permiso...">{{ old('description', $permission->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                            <div>
                                ...
                                <ul class="mt-2 text-sm text-gray-700 space-y-1">
                                    <li><strong>Permiso ID:</strong> {{ $permission->perm_id }}</li>
                                    <li><strong>Roles asignados:</strong> 
                                        <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">
                                            {{ $permission->roles->count() }} rol(es)
                                        </span>
                                    </li>
                                </ul>
                            </div>
